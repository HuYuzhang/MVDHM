...
template<typename T>Void Opt::fill(T** src, T** p, UInt h, UInt w) {
	// Here I use the dp to cal the local sum~ sugoi!
	p[0][0] = src[0][0];
	for (UInt i = 1; i < w; ++i)
	{
...
Void Opt::updateMap(UInt srcPOC, UInt dstPOC)
{
	...
	if (POC2Map.find(srcPOC) != POC2Map.end() && POC2Map[srcPOC].find(dstPOC) != POC2Map[srcPOC].end())
	{
		...
		// I want to make sure that this func is only called once!
		//assert(0);
		return;
	}
	...
	{
		// So for each srcPOC, this block will only enter once~
	...
	//printf("Now update flow from %d to %d\n", srcPOC, dstPOC);
	if (srcPOC == 0 && dstPOC == 1)
	{
		int ooo = 0;
	}
	...
	cmd_string += " 2>&1";// Without this, I will get too many warning... can't bear it!
	//FILE* fp = _popen(cmd_string.c_str(), "r");
	FILE* fp = _popen(cmd_string.c_str(), "r");
	char buf[255];
	while (fgets(buf, 255, fp) != NULL)
	{
		//std::cout << buf; // I don't want to see them!
	}
	_pclose(fp);
	Float Fx, Fy;
	...
	// Init the array part
	POC2Map[srcPOC][dstPOC] = opMap; // I don't know why to store the Mat, maybe for future usage?
	avgPUX[srcPOC][dstPOC] = px;
	avgPUY[srcPOC][dstPOC] = py;
	// And finally init the px and py array
	// Wow what a beautiful code
...
OptInfo Opt::queryFlow(UInt srcPOC, UInt dstPOC, UInt lx, UInt ly, UInt rx, UInt ry)
{
	...
	if (POC2Map.find(srcPOC) != POC2Map.end() && POC2Map[srcPOC].find(dstPOC) != POC2Map[srcPOC].end())
	{
		...
		Float factor = (ry - ly + 1) * (rx - lx + 1);
		Float tmpX = px[ry][rx], tmpY = py[ry][rx];
		if (lx != 0)
		{
			tmpX -= px[ry][lx - 1];
			tmpY -= py[ry][lx - 1];
		...
		return OptInfo(tmpX / factor, tmpY / factor);
	}
	else
	{
		// Here I assert(0) to exit the program... Before I want that each time I call queryFlow, We already calculated it
...
//
//Void Opt::bakCTU(UInt poc, TComDataCU** p, UInt beg, UInt tail)
//{
//	assert(beg == 0);
//	ctuBak[poc] = new TComDataCU*[tail - beg];
//	for (int i = beg; i < tail; ++i)
//	{
//		ctuBak[poc][i] = new TComDataCU();
//
//		//ctuBak[poc][i]->
//	}
//}